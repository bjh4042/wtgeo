...
import { Place, categoryIcons, categoryColors, getDistance, getEstimatedTime, getRoadViewUrl, getDirectionUrl } from '@/data/places';
import { School } from '@/data/schools';
import { getMergedPlacesByGrade } from '@/data/dataManager';
import { X, Plus, Trash2, Navigation, Route, MapPin, ChevronUp, ChevronDown } from 'lucide-react';
...
const RouteExplorer = ({ grade, school, onClose, onPlaceSelect }: RouteExplorerProps) => {
  ...
  const filteredPlaces = useMemo(() => {
    ...
    return allPlaces.filter(p => !ids.has(p.id) && (
      !searchTerm || p.name.includes(searchTerm) || p.address.includes(searchTerm)
    ));
  }, [allPlaces, routePlaces, searchTerm]);
