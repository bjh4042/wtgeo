...
import { GyeongnamCity } from './gyeongnam';
...
// ═══════════════════════════════════════════
// 경남 시군 (Gyeongnam Cities)
// ═══════════════════════════════════════════

const gyeongnamHeaderAliases: Record<string, string> = {
  '시군명': 'name', '이름': 'name', '시군': 'name',
  '한자': 'nameHanja', '한자명': 'nameHanja',
  '인구': 'population', '면적': 'area',
  '마스코트': 'mascot', '마스코트이모지': 'mascotEmoji', '이모지': 'mascotEmoji',
  '마스코트이미지': 'mascotImageUrl',
  '로고': 'logoUrl', '로고url': 'logoUrl',
  '공식사이트': 'officialSite', '홈페이지': 'officialSite',
  '이름유래': 'nameOrigin', '유래': 'nameOrigin',
  '위도': 'lat', '경도': 'lng',
  '주요특징': 'highlights', '특징': 'highlights',
};

/** Excel → GyeongnamCity partial[] 파싱 */
export function parseExcelToGyeongnam(buffer: ArrayBuffer): Partial<GyeongnamCity>[] {
  const rows = readRows(buffer);
  if (rows.length < 2) return [];
  const headerMap = mapHeaders(rows[0].map(String), gyeongnamHeaderAliases);
  const cities: Partial<GyeongnamCity>[] = [];
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const city: Partial<GyeongnamCity> & { name: string } = { name };
    if (data.nameHanja) city.nameHanja = data.nameHanja;
    if (data.population) city.population = parseInt(data.population) || 0;
    if (data.area) city.area = parseFloat(data.area) || 0;
    if (data.mascot) city.mascot = data.mascot;
    if (data.mascotEmoji) city.mascotEmoji = data.mascotEmoji;
    if (data.mascotImageUrl) city.mascotImageUrl = data.mascotImageUrl;
    if (data.logoUrl) city.logoUrl = data.logoUrl;
    if (data.officialSite) city.officialSite = data.officialSite;
    if (data.nameOrigin) city.nameOrigin = data.nameOrigin;
    if (data.lat) city.lat = parseFloat(data.lat);
    if (data.lng) city.lng = parseFloat(data.lng);
    if (data.highlights) city.highlights = data.highlights.split(',').map(s => s.trim()).filter(Boolean);
...
/** GyeongnamCity[] → Excel 다운로드 */
export function exportGyeongnamToExcel(cities: GyeongnamCity[], filename = '거제탐험대_경남시군데이터.xlsx') {
  const headers = ['시군명', '한자', '인구', '면적', '마스코트', '이모지', '공식사이트', '이름유래', '위도', '경도', '주요특징', '로고URL', '마스코트이미지'];
  const data = cities.map(c => [
    c.name, c.nameHanja, c.population, c.area, c.mascot, c.mascotEmoji,
    c.officialSite, c.nameOrigin, c.lat, c.lng,
    c.highlights.join(', '), c.logoUrl || '', c.mascotImageUrl || '',
  ]);
  const ws = XLSX.utils.aoa_to_sheet([headers, ...data]);
  ws['!cols'] = [{ wch: 10 }, { wch: 10 }, { wch: 10 }, { wch: 8 }, { wch: 10 }, { wch: 6 }, { wch: 25 }, { wch: 40 }, { wch: 10 }, { wch: 10 }, { wch: 30 }, { wch: 25 }, { wch: 25 }];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, '경남시군');
  XLSX.writeFile(wb, filename);
}
